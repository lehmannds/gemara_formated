 ...
 *
 * Usage: node scripts/split-mishna-gemara.js <file> [--dry-run]
 */
...
if (!file) {
  console.error('Usage: node scripts/split-mishna-gemara.js <file> [--dry-run]');
  process.exit(1);
}
...
for (let i = 0; i < nodes.length; i++) {
  ...
  // Check if there's already a [mishna] tag right after
  let hasMishna = false;
  let hasGemara = false;
